...
## Design decisions (from John, 2026-09-03)
...
- Research: web search freely during desk research; browser only when John invokes it, one pass per invocation, date-stamped, never continuous.
...
2. Pilot intake — answer open questions in conversation; create companion files if John names the two friends; freeze the brief. Verify: no "TBD" left that affects destination choice.
...
4. Options — `options.md`: three destination-level options with different assumptions (e.g. Lisbon shoulder-calm vs. Barcelona post-Easter vs. Madrid + Valencia day-trip for beach). Verify: totals reconcile to `budget.xlsx`; each option lists its biggest risk against John's hard dislikes.
...
7. Debrief (after trip, ~April 2027) — `debrief.md`; propose profile/companion updates; propose SKILL.md changes. Verify: every profile change traces to a debrief line John confirmed.

## Deliberately not built
- No automated fare alerts or scheduled runs (John wants invocation-only).
